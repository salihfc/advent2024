...
set<Vector2> visited, on_best_path;
Vector2 start_pos, end_pos;


void dfs(Vector2 pos, Vector2 prev, Direction dir, int cost, vector<Vector2>& path)
{
    if (cost > best)
        return;
    ...
    for (int i = 0; i < dirs_to_check.size(); i++)
    {
        ...
        path.push_back(next);
        visited.insert(next);
        dfs(next, pos, next_dir, (cost + 1) + CHANGE_DIR_COST_FACTOR * dir_distance(next_dir, dir), path);
...
int main()
{
    ...
    dfs(start_pos, start_pos, RIGHT, 0, path);
    // dfs(start_pos, start_pos, RIGHT, 0);

    cout << "Best: " << best-1000 << endl;
    cout << "Best pos: " << on_best_path.size() << endl;
}